...
`
	...
	yamlOutput, err := GenerateConfigYamlWithSource(settings, false, true, sourcePath)
	if err != nil {
...
`
	...
	_, secrets, err := CollectCommentsAndSecrets(tmpFile)
	if err != nil {
...
`
	...
	fullYaml, err := GenerateConfigYamlWithSource(config, false, true, sourcePath)
	if err != nil {
		t.Fatalf("GenerateConfigYaml with full=true failed: %v", err)
	}

	// Test full=false - should only show non-default fields
	filteredYaml, err := GenerateConfigYamlWithSource(config, false, false, sourcePath)
	if err != nil {
...
func TestGenerateConfigYaml_CommentsOnOff(t *testing.T) {
	...
	withComments, err := GenerateConfigYaml(config, true, true)
	if err != nil {
		t.Fatalf("GenerateConfigYaml with comments=true failed: %v", err)
	}

	// Test with comments=false using the real source
	withoutComments, err := GenerateConfigYaml(config, false, true)
	if err != nil {
...
func TestGenerateConfigYaml_IntegrationTest(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "Static config mode with comments",
			showComments:   true,
			showFull:       true,
			expectSecret:   true, // secrets should be hidden
			expectFull:     true, // all fields shown
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			yamlOutput, err := GenerateConfigYaml(config, tt.showComments, tt.showFull)
			if err != nil {
				t.Fatalf("GenerateConfigYaml failed: %v", err)
			}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, showComments := range []bool{false, true} {
				for _, showFull := range []bool{false, true} {
					yamlOutput, err := GenerateConfigYaml(tt.config, showComments, showFull)
					if err != nil {
						t.Fatalf("GenerateConfigYaml failed for %s (comments=%v, full=%v): %v",
							tt.desc, showComments, showFull, err)
					}
