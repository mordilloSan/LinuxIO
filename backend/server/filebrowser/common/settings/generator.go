...
// CollectCommentsAndSecrets parses all Go source and returns comments and secret field mappings
func CollectCommentsAndSecrets(srcPath string) (CommentsMap, SecretFieldsMap, error) {
	...
	// From common/settings, go up to backend root, then into auth/users
	var usersDir string
	if filepath.IsAbs(dir) {
		// If absolute path, calculate relative to it
		usersDir = filepath.Join(filepath.Dir(filepath.Dir(dir)), "auth/users")
	} else {
	...
	for _, parseDir := range dirsToparse {
		dirComments, dirSecrets, err := parseDirectoryComments(parseDir)
		if err != nil {
...
// parsePackageFiles reads Go files from a directory and returns them grouped by package name.
...
// parseDirectoryComments parses a single directory for comments and secret fields
func parseDirectoryComments(dir string) (CommentsMap, SecretFieldsMap, error) {
	pkgs, err := parsePackageFiles(dir)
	if err != nil {
		return nil, nil, err
	}
...
// CollectCommentsFromEmbeddedYaml parses comments from embedded YAML content and returns CommentsMap and SecretFieldsMap
func CollectCommentsFromEmbeddedYaml(yamlContent string) (CommentsMap, SecretFieldsMap, error) {
	comments := make(CommentsMap)
	secrets := make(SecretFieldsMap)
...
func buildNodeWithDefaults(v reflect.Value, comm CommentsMap, defaults reflect.Value, secrets SecretFieldsMap) (*yaml.Node, error) {
	...
	case reflect.Map:
		...
		for _, keyStr := range keyStrings {
			...
			valNode, err := buildNodeWithDefaults(mapVal, comm, defaultVal, secrets)
			if err != nil {
				return nil, err
			}
...
func GenerateYaml() {
	...
	// Generate YAML with comments enabled and full config for static output
	// Use multi-directory comment parsing to get comments from all packages
	yamlContent, err := GenerateConfigYamlWithSource(&Config, true, true, "common/settings")
	if err != nil {
...
// GenerateConfigYaml generates YAML from a given config with options for comments and full output
func GenerateConfigYaml(config *Settings, showComments bool, showFull bool) (string, error) {
	...
	for _, sourcePath := range sourcePaths {
		yamlOutput, err := GenerateConfigYamlWithSource(config, showComments, showFull, sourcePath)
		if err == nil {
...
func GenerateConfigYamlWithEmbedded(config *Settings, showComments bool, showFull bool, embeddedYaml string) (string, error) {
	var (
		comm    CommentsMap
	...
	if embeddedYaml != "" {
		comm, secrets, err = CollectCommentsFromEmbeddedYaml(embeddedYaml)
		if err != nil {
...
// GenerateConfigYamlWithSource generates YAML from a given config using a custom source path for comments
func GenerateConfigYamlWithSource(config *Settings, showComments bool, showFull bool, sourcePath string) (string, error) {
	var (
		comm    CommentsMap
		secrets SecretFieldsMap
		err     error
	...
	if showComments {
		comm, secrets, err = CollectCommentsAndSecrets(sourcePath)
		if err != nil {
			return "", err
		}
	} else {
		comm = make(CommentsMap)
		_, secrets, err = CollectCommentsAndSecrets(sourcePath)
		if err != nil {
			return "", err
		}
	}
...
// buildImagePreviewMap is no longer used - media playback removed
func buildImagePreviewMap(v reflect.Value) (*yaml.Node, error) {
	mapNode := &yaml.Node{Kind: yaml.MappingNode}
