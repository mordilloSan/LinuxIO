...
// setDiskUsageCache configures how long non-root filesystem usage is reused.
// Callers must hold the app mutex the collector takes while reading it.
...
// reseedFromCollector replaces a live key's per-device baselines with the
// collector's when the live baselines predate the last collector tick and that
// tick is at least reseedMinWindow old, so a request after idle averages over
// at most one collector interval without collapsing onto a millisecond window.
func (m *fsManager) reseedFromCollector(cacheTimeMs uint16, now time.Time) {
	if cacheTimeMs == collectorDataKeyMs {
		return
	}
	reseedPrevDisk(m.diskPrev, cacheTimeMs, now)
	reseedPrevDisk(m.devicePrev, cacheTimeMs, now)
}

// reseedPrevDisk clones the collector baselines of one prevDisk tracker into
// its live key when that key's baselines are older than the collector's and
// the collector's are old enough to leave a usable delta window.
func reseedPrevDisk(tracker map[uint16]map[string]prevDisk, cacheTimeMs uint16, now time.Time) {
	collector := tracker[collectorDataKeyMs]
	if len(collector) == 0 {
		return
	}
	...
	for _, prev := range tracker[cacheTimeMs] {
		if prev.at.After(liveAt) {
			liveAt = prev.at
		}
	}
	if !shouldReseedFromCollector(liveAt, collectorAt, now) {
		return
	}
	tracker[cacheTimeMs] = maps.Clone(collector)
}
