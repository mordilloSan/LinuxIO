	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedConfig.sensors == nil {
				...
				assert.Len(t, result.sensors, len(tt.expectedConfig.sensors))
...
func TestGetTempsWithPanicRecovery(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			getTempsFn: func(ctx context.Context) ([]sensors.TemperatureStat, error) {
				var i any = "string"
				//nolint:errcheck // the unchecked assertion is the panic under test
				_ = i.(int)
				return nil, nil
			},
			expectError: true,
...
func TestUpdateTemperaturesSkipsOnTimeout(t *testing.T) {
	...
	assert.InDelta(t, 0.0, agent.systemInfoManager.systemInfo.DashboardTemp, 1e-9)
