...
// gpuDataForTest copies a GPU entry under the manager's mutex. Collector
// goroutines create and mutate GpuDataMap entries while holding that same
// lock, so a test running alongside them must not read the map directly. The
// copy is shallow: the returned Engines map still aliases manager state, so it
// must not be read from a test with a collector running.
func gpuDataForTest(gm *GPUManager, id string) (system.GPUData, bool) {
	gm.Lock()
	defer gm.Unlock()
	gpu, ok := gm.GpuDataMap[id]
	if !ok {
		return system.GPUData{}, false
	}
	return *gpu, true
}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantValid {
				for id, want := range tt.wantData {
					...
					assert.InDelta(t, want.Count, got.Count, 1e-9)
				}
			}
		})
	}
...
func TestParseNvtopData(t *testing.T) {
	...
	assert.InDelta(t, 48.0, g0.Temperature, 1e-9)
	assert.InDelta(t, 5.0, g0.Usage, 1e-9)
	assert.InDelta(t, 13.0, g0.Power, 1e-9)
	assert.InDelta(t, utils.BytesToMegabytes(349372416), g0.MemoryUsed, 1e-9)
	assert.InDelta(t, utils.BytesToMegabytes(4294967296), g0.MemoryTotal, 1e-9)
	assert.InDelta(t, 1.0, g0.Count, 1e-9)

	g1, ok := gm.GpuDataMap["n1"]
	require.True(t, ok)
	assert.Equal(t, "AMD Radeon 680M", g1.Name)
	assert.InDelta(t, 48.0, g1.Temperature, 1e-9)
	assert.InDelta(t, 12.0, g1.Usage, 1e-9)
	assert.InDelta(t, 9.0, g1.Power, 1e-9)
	assert.InDelta(t, utils.BytesToMegabytes(1213784064), g1.MemoryUsed, 1e-9)
	assert.InDelta(t, utils.BytesToMegabytes(16929173504), g1.MemoryTotal, 1e-9)
	assert.InDelta(t, 1.0, g1.Count, 1e-9)
}
...
func TestUpdateNvtopSnapshotsKeepsDeviceAssociationWhenOrderChanges(t *testing.T) {
	...
	assert.InDelta(t, 70.0, nvidia.Usage, 1e-9)
	assert.InDelta(t, 25.0, nvidia.Power, 1e-9)
	assert.InDelta(t, 2.0, nvidia.Count, 1e-9)

	amd := gm.GpuDataMap["n1"]
	require.NotNil(t, amd)
	assert.Equal(t, "AMD Radeon 680M", amd.Name)
	assert.InDelta(t, 70.0, amd.Usage, 1e-9)
	assert.InDelta(t, 45.0, amd.Power, 1e-9)
	assert.InDelta(t, 2.0, amd.Count, 1e-9)
}

func TestUpdateNvtopProcessSnapshotsParsesKnownZeroAndClearsStaleSamples(t *testing.T) {
	raw := func(value string) json.RawMessage { return json.RawMessage(value) }
	gm := &GPUManager{GpuDataMap: make(map[string]*system.GPUData)}

	require.True(t, gm.updateNvtopProcessSnapshots([]nvtopSnapshot{{
		DeviceName: "Example GPU",
		Processes: []nvtopProcess{{
			PID:              raw(`"42"`),
			GPUUsage:         raw(`"0%"`),
			GPUMemBytesAlloc: raw(`"0"`),
			Encode:           raw(`"10%"`),
			Decode:           raw(`"20%"`),
		}},
	}}))
	assert.Empty(t, gm.GpuDataMap, "process-only snapshots must not alter aggregate GPU data")

	samples := gm.GPUProcessSamples()
	require.Len(t, samples, 1)
	assert.Equal(t, int32(42), samples[0].PID)
	require.NotNil(t, samples[0].GPUUtilization)
	assert.Zero(t, *samples[0].GPUUtilization)
	require.NotNil(t, samples[0].MemoryBytes)
	assert.Zero(t, *samples[0].MemoryBytes)
	require.NotNil(t, samples[0].EncodeUtilization)
	assert.InDelta(t, 10.0, *samples[0].EncodeUtilization, 1e-9)
	require.NotNil(t, samples[0].DecodeUtilization)
	assert.InDelta(t, 20.0, *samples[0].DecodeUtilization, 1e-9)

	require.True(t, gm.updateNvtopProcessSnapshots([]nvtopSnapshot{{DeviceName: "Example GPU"}}))
	assert.Empty(t, gm.GPUProcessSamples(), "an empty process list must remove exited processes")
}

func TestNvtopProcessReplacementPreservesOtherSourcesAndPreference(t *testing.T) {
	raw := func(value string) json.RawMessage { return json.RawMessage(value) }
	gm := &GPUManager{
		GpuDataMap:         make(map[string]*system.GPUData),
		processSamples:     make(map[string][]GPUProcessSample),
		processSourceReady: make(map[string]bool),
	}
	gm.Lock()
	gm.replaceGPUProcessSourceLocked(string(collectorSourceNVML), map[string][]GPUProcessSample{
		"0": {{DeviceID: "0", DeviceName: "Example GPU", PID: 42, Source: string(collectorSourceNVML)}},
	}, true)
	gm.Unlock()
	require.True(t, gm.updateNvtopProcessSnapshots([]nvtopSnapshot{{
		DeviceName: "Example GPU",
		Processes:  []nvtopProcess{{PID: raw(`"42"`), GPUUsage: raw(`"25%"`)}},
	}}))
	assert.Len(t, gm.GPUProcessSamples(), 2, "source-scoped replacement must preserve NVML fallback samples")

	gm.setGPUProcessSourcePreference(nvtopCmd)
	preferred := gm.GPUProcessSamples()
	require.Len(t, preferred, 1)
	assert.Equal(t, nvtopCmd, preferred[0].Source)

	gm.clearGPUProcessSource(nvtopCmd)
	fallback := gm.GPUProcessSamples()
	require.Len(t, fallback, 1)
	assert.Equal(t, string(collectorSourceNVML), fallback[0].Source)
}
...
func TestParseJetsonData(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.InDelta(t, tt.wantMetrics.Count, got.Count, 1e-9)
		})
	}
}
...
func TestGetCurrentData(t *testing.T) {
	t.Run("calculates averages with per-cache-key delta tracking", func(t *testing.T) {
		...
		assert.InDelta(t, 2, gm.GpuDataMap["0"].Count, 1e-9, "GPU 0 Count should remain at 2")
		assert.InDelta(t, 100, gm.GpuDataMap["0"].Usage, 1e-9, "GPU 0 Usage should remain at 100")
		assert.InDelta(t, 200.0, gm.GpuDataMap["0"].Power, 1e-9, "GPU 0 Power should remain at 200")
		assert.InDelta(t, 1.0, gm.GpuDataMap["1"].Count, 1e-9, "GPU 1 Count should remain at 1")
		assert.InDelta(t, 30.0, gm.GpuDataMap["1"].Usage, 1e-9, "GPU 1 Usage should remain at 30")
		assert.InDelta(t, 60.0, gm.GpuDataMap["1"].Power, 1e-9, "GPU 1 Power should remain at 60")
		...
		assert.InDelta(t, 100.0, gm.lastSnapshots[cacheKey]["0"].usage, 1e-9)
		assert.InDelta(t, 200.0, gm.lastSnapshots[cacheKey]["0"].power, 1e-9)
	})
	...
	t.Run("handles zero count without panicking", func(t *testing.T) {
		...
		assert.InDelta(t, 0.0, result["0"].Usage, 1e-9)
		assert.InDelta(t, 0.0, result["0"].Power, 1e-9)

		// Verify count remains 0
		assert.InDelta(t, 0, gm.GpuDataMap["0"].Count, 1e-9)
	})
	...
	t.Run("uses last average when no new data arrives", func(t *testing.T) {
		...
		assert.InDelta(t, 2, gm.GpuDataMap["0"].Count, 1e-9, "Count should remain at 2")
		...
		assert.InDelta(t, 2, gm.GpuDataMap["0"].Count, 1e-9, "Count should still be 2")
	})
...
func TestCalculateDeltas(t *testing.T) {
	...
	t.Run("with no previous snapshot", func(t *testing.T) {
		...
		assert.InDelta(t, 100.5, deltaUsage, 1e-9)
		assert.InDelta(t, 250.75, deltaPower, 1e-9)
		assert.InDelta(t, 300.25, deltaPowerPkg, 1e-9)
	})
...
func TestCalculateIntelGPUUsage(t *testing.T) {
	...
	t.Run("with no previous snapshot", func(t *testing.T) {
		...
		assert.InDelta(t, 40.0, maxUsage, 1e-9, "Should return max engine usage (80/2=40)")
		assert.InDelta(t, 40.0, gpuAvg.Engines["Render/3D"], 1e-9)
		assert.InDelta(t, 20.0, gpuAvg.Engines["Video"], 1e-9)
		assert.InDelta(t, 30.0, gpuAvg.Engines["Compute"], 1e-9)
	})
	...
	t.Run("with previous snapshot", func(t *testing.T) {
		...
		assert.InDelta(t, 20.0, maxUsage, 1e-9, "Should return max engine delta (100/5=20)")
		assert.InDelta(t, 20.0, gpuAvg.Engines["Render/3D"], 1e-9)
		assert.InDelta(t, 12.0, gpuAvg.Engines["Video"], 1e-9)
		assert.InDelta(t, 16.0, gpuAvg.Engines["Compute"], 1e-9)
	})
	...
	t.Run("handles missing engine in snapshot", func(t *testing.T) {
		...
		assert.InDelta(t, 25.0, maxUsage, 1e-9)
		assert.InDelta(t, 10.0, gpuAvg.Engines["Render/3D"], 1e-9, "Should use delta for existing engine")
		assert.InDelta(t, 25.0, gpuAvg.Engines["NewEngine"], 1e-9, "Should use full value for new engine")
	})
}
...
func TestUpdateInstantaneousValues(t *testing.T) {
	...
	t.Run("updates temperature, memory used and total", func(t *testing.T) {
		...
		assert.InDelta(t, 75.57, gpuAvg.Temperature, 1e-9, "Should update and round temperature")
		assert.InDelta(t, 2500.89, gpuAvg.MemoryUsed, 1e-9, "Should update and round memory used")
		assert.InDelta(t, 8192.23, gpuAvg.MemoryTotal, 1e-9, "Should update and round memory total")
	})
}
...
func TestStoreSnapshot(t *testing.T) {
	...
	t.Run("stores standard GPU snapshot", func(t *testing.T) {
		...
		assert.InDelta(t, 150.5, snapshot.usage, 1e-9)
		assert.InDelta(t, 250.75, snapshot.power, 1e-9)
		assert.InDelta(t, 300.25, snapshot.powerPkg, 1e-9)
	...
	t.Run("stores Intel GPU snapshot with engines", func(t *testing.T) {
		...
		assert.InDelta(t, 80.0, snapshot.engines["Render/3D"], 1e-9)
		assert.InDelta(t, 40.0, snapshot.engines["Video"], 1e-9)
	...
	t.Run("overwrites existing snapshot", func(t *testing.T) {
		...
		assert.InDelta(t, 250.0, snapshot.usage, 1e-9, "Should overwrite previous usage")
		assert.InDelta(t, 400.0, snapshot.power, 1e-9, "Should overwrite previous power")
	})
}
...
func TestCalculateGPUAverage(t *testing.T) {
	t.Run("returns cached average when deltaCount is zero", func(t *testing.T) {
		...
		assert.InDelta(t, 50.0, result.Usage, 1e-9, "Should return cached average")
		assert.InDelta(t, 100.0, result.Power, 1e-9, "Should return cached average")
	})
	...
	t.Run("returns zero value when GPU is suspended", func(t *testing.T) {
		...
		assert.InDelta(t, 0.0, result.Usage, 1e-9, "Should return zero usage")
		assert.InDelta(t, 0.0, result.Power, 1e-9, "Should return zero power")
	})
	...
	t.Run("calculates average for standard GPU", func(t *testing.T) {
		...
		assert.InDelta(t, 50.0, result.Usage, 1e-9)
		assert.InDelta(t, 100.0, result.Power, 1e-9)
	...
	t.Run("calculates average for Intel GPU with engines", func(t *testing.T) {
		...
		assert.InDelta(t, 100.0, result.Power, 1e-9)
		assert.InDelta(t, 120.0, result.PowerPkg, 1e-9)
		assert.InDelta(t, 20.0, result.Usage, 1e-9, "Should use max engine usage")
		assert.InDelta(t, 20.0, result.Engines["Render/3D"], 1e-9)
		assert.InDelta(t, 10.0, result.Engines["Video"], 1e-9)
	})
	...
	t.Run("calculates delta from previous snapshot", func(t *testing.T) {
		...
		assert.InDelta(t, 30.0, result.Usage, 1e-9)
		assert.InDelta(t, 50.0, result.Power, 1e-9)
	})
...
//nolint:gocognit // upstream go-monitoring test, kept whole
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			hasPriority := func(source collectorSource) bool {
				return slices.Contains(priorities, source)
			}
...
echo "0, NVIDIA Test GPU, 50, 1024, 4096, 25, 100"`
			...
			validate: func(t *testing.T, gm *GPUManager) {
				gpu, exists := gpuDataForTest(gm, "0")
				assert.True(t, exists)
				if exists {
					assert.Equal(t, "Test GPU", gpu.Name)
					assert.InDelta(t, 50.0, gpu.Temperature, 1e-9)
...
echo '{"card0": {"Temperature (Sensor edge) (C)": "49.0", "Current Socket Graphics Package Power (W)": "28.159", "GPU use (%)": "0", "VRAM Total Memory (B)": "536870912", "VRAM Total Used Memory (B)": "445550592", "Card Series": "Rembrandt [Radeon 680M]", "Card Model": "0x1681", "Card Vendor": "Advanced Micro Devices, Inc. [AMD/ATI]", "Card SKU": "REMBRANDT", "Subsystem ID": "0x8a22", "Device Rev": "0xc8", "Node ID": "1", "GUID": "34756", "GFX Version": "gfx1035"}}'`
			...
			validate: func(t *testing.T, gm *GPUManager) {
				gpu, exists := gpuDataForTest(gm, "34756")
				assert.True(t, exists)
				if exists {
...
echo '[{"device_name":"NVIDIA Test GPU","temp":"52C","power_draw":"31W","gpu_util":"37%","mem_total":"4294967296","mem_used":"536870912","processes":[]}]'`
			...
			validate: func(t *testing.T, gm *GPUManager) {
				gpu, exists := gpuDataForTest(gm, "n0")
				assert.True(t, exists)
				if exists {
					assert.Equal(t, "NVIDIA Test GPU", gpu.Name)
					assert.InDelta(t, 52.0, gpu.Temperature, 1e-9)
				}
			},
		},
	}
...
echo "0, NVIDIA Priority GPU, 45, 512, 2048, 12, 25"`
	...
	gpu, ok := gpuDataForTest(gm, "0")
	require.True(t, ok)
	assert.Equal(t, "Priority GPU", gpu.Name)
	assert.InDelta(t, 45.0, gpu.Temperature, 1e-9)
}
...
`
	...
	time.Sleep(150 * time.Millisecond)
	_, intelOk := gpuDataForTest(gm, "i0")
	_, amdOk := gpuDataForTest(gm, "34756")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for id, expected := range tt.expectedValues {
				...
				assert.InDelta(t, expected.temperature, gpu.Temperature, 1e-9, "Temperature should match")
				assert.InDelta(t, expected.memoryUsed, gpu.MemoryUsed, 1e-9, "Memory used should match")
				assert.InDelta(t, expected.memoryTotal, gpu.MemoryTotal, 1e-9, "Memory total should match")
				assert.InDelta(t, expected.usage, gpu.Usage, 1e-9, "Usage should match")
				assert.InDelta(t, expected.power, gpu.Power, 1e-9, "Power should match")
				assert.InDelta(t, expected.count, gpu.Count, 1e-9, "Count should match")
			}
			...
			for id, expected := range tt.expectedValues {
				...
				assert.InDelta(t, expected.temperature, gpu.Temperature, 1e-9, "Temperature in GetCurrentData should match")
				assert.InDelta(t, expected.avgUsage, gpu.Usage, 1e-9, "Average usage in GetCurrentData should match")
				assert.InDelta(t, expected.avgPower, gpu.Power, 1e-9, "Average power in GetCurrentData should match")
			}
			...
			for id, expected := range tt.expectedValues {
				...
				assert.InDelta(t, expected.count, gpu.Count, 1e-9, "Count should remain at accumulated value for GPU ID %s", id)
				assert.InDelta(t, expected.usage, gpu.Usage, 1e-9, "Usage should remain at accumulated value for GPU ID %s", id)
				assert.InDelta(t, expected.power, gpu.Power, 1e-9, "Power should remain at accumulated value for GPU ID %s", id)
			}
		})
	}
}
...
func TestIntelUpdateFromStats(t *testing.T) {
	...
	assert.Equal(t, "GPU", gpu.Name)
	assert.InDelta(t, 10.5, gpu.Power, 1e-9)
	assert.InDelta(t, 20.0, gpu.Engines["Render/3D"], 1e-9)
	assert.InDelta(t, 5.0, gpu.Engines["Video"], 1e-9)
	assert.InDelta(t, float64(1), gpu.Count, 1e-9)
	...
	require.NotNil(t, gpu)
	assert.InDelta(t, 10.5, gpu.Power, 1e-9)
	assert.InDelta(t, 30.0, gpu.Engines["Render/3D"], 1e-9) // 20 + 10
	assert.InDelta(t, 7.5, gpu.Engines["Video"], 1e-9)      // 5 + 2.5
	assert.InDelta(t, 1.0, gpu.Engines["Blitter"], 1e-9)
	assert.InDelta(t, float64(2), gpu.Count, 1e-9)
}
...
echo "298  295      278  51  2.20  3.12   1675    942   5.75    1   2    9.50    3   1    12.00  1   0"`
	...
	assert.InDelta(t, 6.0, gpu.Power, 1e-9)
	assert.InDelta(t, 8.26, gpu.PowerPkg, 0.01) // Allow small floating point differences
	// Engines aggregated from samples 2-4
	assert.InDelta(t, 14.25, gpu.Engines["Render/3D"], 1e-9) // 0.00 + 8.50 + 5.75
	assert.InDelta(t, 34.0, gpu.Engines["Video"], 1e-9)      // 0.00 + 22.00 + 12.00
	assert.InDelta(t, 24.5, gpu.Engines["Blitter"], 1e-9)    // 0.00 + 15.00 + 9.50
	// Count should be 3 samples (first is skipped)
	assert.InDelta(t, float64(3), gpu.Count, 1e-9)
}
...
func TestParseIntelData(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.InDelta(t, tt.wantPowerGPU, sample.PowerGPU, 1e-9)
