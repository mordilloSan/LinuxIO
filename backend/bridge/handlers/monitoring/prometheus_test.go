...
func TestGetGPUSeriesReportsUnconfiguredHistory(t *testing.T) {
	result := GetGPUSeries(context.Background(), "1m")

	require.False(t, result.Available)
	require.Equal(t, "1m", result.Range)
	require.Equal(t, 5, result.StepSeconds)
	require.Equal(t, "historical GPU metrics are not configured in the monitoring stack", result.Reason)
}
