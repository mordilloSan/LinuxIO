...
var (
	// activeUploads tracks ongoing chunked uploads by RequestID
	activeUploads = make(map[string]*uploadState)
...
// ============================================================================
// STREAMING HANDLERS (for IPC chunking)
// ============================================================================

// StreamingUploadChunk handles file upload chunks received via IPC
// This is exported so the main bridge handler can call it when detecting streaming requests
func StreamingUploadChunk(req *ipc.Request) (*ipc.Response, error) {
	if len(req.Args) < 1 {
		return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: "missing destination path"}, nil
	}

	destPath := req.Args[0]
	override := len(req.Args) > 1 && req.Args[1] == "true"

	uploadsMu.Lock()
	state, exists := activeUploads[req.RequestID]
	uploadsMu.Unlock()

	removeState := func() {
		uploadsMu.Lock()
		delete(activeUploads, req.RequestID)
		uploadsMu.Unlock()
	}
	...
	if !exists {
		if req.Offset != 0 {
			return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: "first chunk must have offset 0"}, nil
		}

		// Create temp directory for uploads
		tempDir := filepath.Join(os.TempDir(), "linuxio-uploads")
		if err := os.MkdirAll(tempDir, 0700); err != nil {
			return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("failed to create temp dir: %v", err)}, nil
		}

		// Create temp file
		tempFile, err := os.CreateTemp(tempDir, "upload-*.tmp")
		if err != nil {
			return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("failed to create temp file: %v", err)}, nil
		}
		...
		uploadsMu.Lock()
		activeUploads[req.RequestID] = state
		uploadsMu.Unlock()

		logger.DebugKV("started chunked upload", "request_id", req.RequestID, "dest", destPath, "total", req.Total)
	}

	state.mu.Lock()

	// Validate offset matches expected position
	if req.Offset != state.BytesWritten {
		state.mu.Unlock()
		return &ipc.Response{
			Status:    "error",
			RequestID: req.RequestID,
			Error:     fmt.Sprintf("offset mismatch: expected %d, got %d", state.BytesWritten, req.Offset),
		}, nil
	}

	// Decode and write chunk
	if req.Payload != "" {
		chunkData, err := base64.StdEncoding.DecodeString(req.Payload)
		if err != nil {
			state.mu.Unlock()
			return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("invalid base64 payload: %v", err)}, nil
		}

		n, err := state.File.Write(chunkData)
		if err != nil {
			state.File.Close()
			os.Remove(state.TempFile)
			state.mu.Unlock()
			removeState()
			return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("failed to write chunk: %v", err)}, nil
		}
	...
	// Final chunk - finalize upload
	if req.Final {
		state.File.Close()

		// Clean destination path
		realDest := filepath.Join(state.DestPath)
		...
		if stat, statErr := os.Stat(realDest); statErr == nil {
			if stat.IsDir() {
				...
				state.mu.Unlock()
				removeState()
				return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: "destination is a directory"}, nil
			}
			if !state.Override {
				os.Remove(state.TempFile)
				state.mu.Unlock()
				removeState()
				return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: "file already exists"}, nil
			}
		}
		...
		if err := os.MkdirAll(filepath.Dir(realDest), services.PermDir); err != nil {
			...
			state.mu.Unlock()
			removeState()
			return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("failed to create parent directory: %v", err)}, nil
		}

		// Move temp file to destination
		if err := os.Rename(state.TempFile, realDest); err != nil {
			// If rename fails (cross-device), copy and delete
			if err := services.CopyFile(state.TempFile, realDest); err != nil {
				os.Remove(state.TempFile)
				state.mu.Unlock()
				removeState()
				return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("failed to copy file: %v", err)}, nil
			}
			os.Remove(state.TempFile)
		}
		...
		logger.DebugKV("completed chunked upload", "request_id", req.RequestID, "dest", realDest, "bytes", bytesWritten)
...
// CleanupStaleUploads removes abandoned uploads older than timeout
func CleanupStaleUploads(timeout time.Duration) {
	now := time.Now()
	var stale []*uploadState

	uploadsMu.Lock()
	for reqID, state := range activeUploads {
		state.mu.Lock()
		expired := now.Sub(state.LastActivity) > timeout
		state.mu.Unlock()
		if expired {
			delete(activeUploads, reqID)
	...
	for _, state := range stale {
		state.mu.Lock()
		logger.DebugKV("cleaning up stale upload", "request_id", state.RequestID, "dest", state.DestPath)
		if state.File != nil {
			state.File.Close()
		}
		os.Remove(state.TempFile)
...
// StreamingDownloadChunk handles file download chunk requests via IPC
// This reads a file in chunks and sends them back base64-encoded
func StreamingDownloadChunk(req *ipc.Request) (*ipc.Response, error) {
	// Validate streaming request (basic validation - offset/total checked differently for downloads)
	if req.RequestID == "" {
		return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: "missing request_id"}, nil
	}
	if req.Offset < 0 {
		return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("invalid offset: %d", req.Offset)}, nil
	}

	if len(req.Args) < 1 {
		return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: "missing file path"}, nil
	}
	...
	stat, err := os.Stat(realPath)
	if err != nil {
		return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("file not found: %v", err)}, nil
	}
	if stat.IsDir() {
		return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: "path is a directory"}, nil
	}

	// Open file for reading
	file, err := os.Open(realPath)
	if err != nil {
		return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("failed to open file: %v", err)}, nil
	}
	defer file.Close()

	// Seek to requested offset
	if req.Offset > 0 {
		if _, seekErr := file.Seek(req.Offset, 0); seekErr != nil {
			return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("failed to seek: %v", seekErr)}, nil
		}
	}
	...
	if err != nil && err != io.EOF {
		return &ipc.Response{Status: "error", RequestID: req.RequestID, Error: fmt.Sprintf("failed to read file: %v", err)}, nil
	}
	...
	final := (req.Offset + int64(n)) >= stat.Size()

	logger.DebugKV("sending download chunk",
