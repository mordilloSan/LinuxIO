...
func pickCommand() (Manager, string, []string) {
    switch {
    case seen("apt"): return MgrAPT, "apt", []string{"list", "--upgradable"}
    case seen("dnf"): return MgrDNF, "dnf", []string{"check-update", "-q", "--refresh"}
    case seen("yum"): return MgrYUM, "yum", []string{"check-update", "-q"}
    case seen("zypper"): return MgrZypper, "zypper", []string{"-q", "lu", "-s"}
    case seen("pacman"): return MgrPacman, "bash", []string{"-lc", "pacman -Sup --noconfirm 2>/dev/null || true"}
    case seen("apk"): return MgrAPK, "apk", []string{"list", "-u"}
    default: return "", "apt", []string{"list", "--upgradable"}
    }
}
...
func parseAptListUpgradeable(out string) []UpdateItem {
    ...
    for sc.Scan() {
        ...
        if !strings.Contains(line, "[upgradable from:") { continue }
...
func parseDnfYum(out string) []UpdateItem {
    ...
    for sc.Scan() {
        ...
        if line == "" || strings.HasPrefix(strings.ToLower(line), "last metadata") ||
            ...
            strings.HasPrefix(line, "Loaded plugins") { continue }
...
func parseZypper(out string) []UpdateItem {
    ...
    for sc.Scan() {
        line := strings.TrimSpace(sc.Text())
        if line == "" { continue }
        if strings.HasPrefix(line, "S |") { header = true; continue }
        if !header || strings.HasPrefix(line, "--") { continue }
...
func parsePacmanSup(out string) []UpdateItem {
    ...
    for sc.Scan() {
        line := strings.TrimSpace(sc.Text())
        if line == "" { continue }
        if !strings.Contains(line, ".pkg.tar.") { continue }
        base := filepath.Base(line)
        base = strings.TrimSuffix(base, ".sig")
        for _, suf := range []string{".pkg.tar.zst", ".pkg.tar.xz", ".pkg.tar.gz", ".pkg.tar"} {
            if strings.HasSuffix(base, suf) { base = strings.TrimSuffix(base, suf); break }
        }
