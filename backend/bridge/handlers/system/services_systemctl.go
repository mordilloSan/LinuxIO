...
func FetchServices() ([]ServiceInfo, error) {
    ...
    for _, b := range blocks {
        ...
        if info.Name == "" || !strings.HasSuffix(info.Name, ".service") { continue }
...
func parseServiceBlock(b []byte) ServiceInfo {
    ...
    for sc.Scan() {
        ...
        switch k {
        case "Id": s.Name = v
        case "ActiveState": s.ActiveState = v
        case "SubState": s.SubState = v
        case "Description": s.Description = v
        case "MainPID": if n, err := strconv.ParseInt(v, 10, 32); err == nil { s.MainPID = int32(n) }
        }
    }
    return s
}
