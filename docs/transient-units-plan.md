# Bridge-Survivable Tasks via systemd Transient Units

## Summary

Long-running Tasks today live entirely inside the per-session bridge process as
goroutines in a package-global service ([`bridge.DefaultTaskService`](../backend/common/ipc/bridge/tasks.go)).
When the bridge dies — session GC, websocket loss, `ReasonBridgeFailure` — every
in-flight Task dies with it. This is the open `ToDo` item #5:
...
For the subset of Tasks that must outlive a session, hand the work to a **systemd
...
- Moving the Task *control plane* into the webserver/session manager. That is a
  separate, larger change; see "Relation to broader architecture".
- Making every Task survivable. Most Tasks are tied to session intent and should
  keep dying with the session.
- Replacing the in-process `TaskService` for session-bound Tasks.

## Background: how Tasks run today

- A request hits the bridge router, which builds a `TaskOwner{SessionID, Username,
  UID}` from the session ([`ownerFromSession`](../backend/common/ipc/bridge/router.go))
  and calls `taskService.CreateForOwner(...)` then `task.Start(runner)`.
...
The Task service already models per-session/user ownership, so the data model is
ready for survivable + reattachable Tasks; only the *execution substrate* is
process-bound.
...
boundary, and splits today's Tasks into two shapes:

1. **Already-subprocess Tasks** — work that already shells out can be wrapped in a
...
2. **In-process Go Tasks** — work implemented against a library (compose via the
   ...
   `linuxio-bridge task-exec --type docker.compose --payload <file>`, which the
   transient unit's `ExecStart` invokes. This is the disciplined version of the
   earlier "each Task a standalone process" idea — one validated binary, many
...
proven, and only for Task types that justify it.
...
- `name`: deterministic, owner-scoped, e.g. `linuxio-<tasktype>-<taskid>.service`.
...
- **Logs:** read the unit's journal by name (`journalctl --unit=<name> --follow`
  or sd-journal with a `_SYSTEMD_UNIT` match) and republish as Task progress
  events through the existing `Task.ReportProgress` / data path.
- **Structured progress / result:** journald is line-oriented text. For typed
  progress and a final result, have the worker write NDJSON to a known per-Task
  path (e.g. `/run/linuxio/tasks/<taskid>.ndjson`) or a unix socket the bridge
  tails. The unit's exit code gives terminal success/failure as a fallback.

### Tracking and reattachment

- Keep a thin in-bridge record mapping `taskID -> unit name` (and the journal
  cursor last published).
- On bridge startup, list units matching the `linuxio-<tasktype>-*` pattern (or
  filter by a custom unit property), rebuild Task records for any still
...
- Task TTL / cleanup → rely on `CollectMode=inactive-or-failed` plus an explicit
  `ResetFailedUnit` sweep (already wrapped in
  [systemd.go](../backend/bridge/handlers/systemd/systemd.go)).
- Absolute caps still apply: a unit should carry `RuntimeMaxSec` so a wedged Task
  cannot run forever.
...
## Task classification (decide before building)

Produce a short table of every `ModeTask` / Task runner route and mark each:
`survivable?` / `idempotent or resumable?` / `already a subprocess?`. Only
`survivable` types get a transient unit; everything else stays an in-process Task.
...
2. **One real Task** — migrate app update (`control.app_update`): wrap the existing
...
3. **Reattach** — implement unit re-discovery + journal-cursor resume on bridge
   restart; verify a Task started pre-restart finishes and reports post-restart.
4. **Generalize** — extract a `RunAsTransientUnit(task, spec)` helper and migrate
   the other shape-(1) candidates.
5. **Worker subcommand (optional)** — add `linuxio-bridge task-exec` and migrate a
   shape-(2) Task (e.g. `docker.compose`) only if justified.
...
- **systemd availability.** Containers/minimal hosts may lack a usable system
  ...
  and fall back to the current in-process Task for non-survivable execution.
...
- **Cross-session ownership.** With `SingleSessionPerUser = false`, two sessions
  ...
  `TaskOwner.Matches`, not just username.
...
webserver/session manager own a durable Task index and central log. It also pairs
with the [Session Activity Timeout Plan](./session-activity-timeout-plan.md):
transient-unit Tasks no longer need to inhibit bridge idle GC to stay alive,
simplifying that design for survivable types. Prerequisite cleanup: the former
`ToDo` item for reviewing the background-work core should land first so this is
built on understood foundations.
