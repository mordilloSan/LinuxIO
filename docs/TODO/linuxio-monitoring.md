...
> **Status: Approved design, not implemented.** LinuxIO currently consumes
> go-monitoring as an optional external capability installed from its APT
> repository. This plan imports it as `linuxio-monitoring`, a binary shipped
> with LinuxIO that owns every sampled measurement of the host, live and
> historical.
...
| Optional listeners | Configured `listeners: [{name, address, apis: ["metrics"]}]`, default empty. TCP or unix. Validation rejects `commands` and `debug`. Unix sockets keep mode 0660. Unauthenticated, as before. |
| Database | `/var/lib/linuxio/monitoring/metrics.db` |
| Config | `/etc/linuxio/monitoring/config.json`, version 1, created by the daemon when absent |
...
2. **Stale baseline reseeds from the collector.** When a live key's baseline is
   older than the last collector tick, the baseline is replaced by that tick's
   counters before the delta is computed. A first request after an idle period
   averages over at most one collector interval instead of the whole idle
   time, and a key's first network sample has a real window. Applies in the cpu,
   network, disk and docker managers.
...
```json
{
  ...
  "history": "cpu,mem,swap,diskio,network,containers,container_telemetry"
}
```

`allow_remote_commands` is removed: commands are served only on the control
socket, which sits behind LinuxIO login and the bridge's privileged-route check.
The two fixed sockets are not configurable and do not appear in `listeners`.
SIGHUP reload and the `config.set` live-apply behaviour stay.
...
| `system.get_processes` | Unchanged. Reserved for a future process view backed by the daemon's processes plugin. |
...
- Settings section: remote-commands toggle removed; the listener editor holds
  address entries only, with the API fixed to metrics and a note that listeners
  are unauthenticated; status shows the fixed sockets.
...
**Kept as is:** collectors, docker integration, store and history aggregation,
JSON config loader and SIGHUP reload, command API, HTTP server and listener
code, collector handoff extended as described above.
...
the systemd plugin and its domain package, its version package, `packaging/`
and `scripts/`, `allow_remote_commands`, and the implicit loopback-TCP default
listener.
...
- New daemon tests: one-second reuse and concurrent sharing on a live key;
  ...
  `api.Live` builder from fixed inputs; listener validation rejecting
  `commands` and `debug` on configured listeners; the peer-uid gate on the
  control socket; the moved sensors parser, GPU readers, disk filter and
  filesystem listing against their existing fixtures.
- New bridge tests: `monitoring.get_live` against a fake daemon including the
  unreachable case; container metrics from the live block; capability
  detection; static route caching; CLI log filters.
...
- Replace the raw Docker Engine client with the moby client the bridge uses.
- Read fans and voltages from hwmon sysfs instead of `sensors -j`.
- Process view backed by the daemon's processes and programs plugins.
- YAML config in the indexer's style if configuration consistency becomes a
  goal.
