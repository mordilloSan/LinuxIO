...
## Orchestration and Luna delegation
...
- For non-trivial work, prefer Luna (`gpt-5.6-luna`) workers for bounded
  exploration, implementation, and test execution when delegation saves
  main-context time. Tiny changes do not require a subagent when spawning one
...
- After non-trivial implementation is quiescent, Sol should spawn a fresh Luna
  test worker to run the required final Make target and report the exact result.
  Luna supplies test evidence; it does not approve the change or own the final
  diff review.
...
- Always use repository Make targets. Do not bypass them with raw `npm`,
  `eslint`, `tsc`, `go test`, `gofmt`, or `golangci-lint` commands unless
  diagnosing a Make target failure.
- Frontend-only changes: run `make check-frontend`.
- Backend-only changes: run `make check-backend`.
- Changes spanning frontend and backend, generated API contracts, shared build
  tooling, or unclear ownership boundaries: run `make test`.
- For claims that depend on real browser navigation, chunk loading, or browser
  ...
  `make setup-frontend-browser` first only when its browser dependency is not
  already installed.
...
- When Go-owned API contracts or generator inputs change, use `make generate`
  and then validate the combined result with `make test`.
