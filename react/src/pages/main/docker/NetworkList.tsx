...
import { Box, Typography, Chip, Table, TableBody, TableCell, TableRow, TableHead, Paper } from "@mui/material";
        ...
        <Box>
            ...
            <Box sx={{ mb: 2 }}>
                {nw.IPAM?.Config && nw.IPAM.Config.length > 0
                    ...
                    : (
                        <Typography variant="body2" color="text.secondary">(no IPAM config)</Typography>
                    )}
            </Box>
                ...
                {containers && containers.length > 0 ? (
                    <Paper sx={{ mb: 2, background: "transparent", boxShadow: "none" }}>
                        <Table size="small">
                            <TableHead>
                                <TableRow>
                                    {["Name", "Container ID", "IPv4", "IPv6", "MAC"].map((header) => (
                                        <TableCell key={header} sx={{ borderBottom: "none", background: "transparent" }}>
                                            <Chip
                                                label={header}
                                                size="small"
                                            />
                            ...
                            <TableBody>
                                {containers.map(([id, info]: [string, any]) => (
                                    <TableRow key={id}>
                                        ...
                                        <TableCell>{info.IPv4Address?.replace(/\/.*/, "") || "-"}</TableCell>
                                        <TableCell>{info.IPv6Address?.replace(/\/.*/, "") || "-"}</TableCell>
                    ...
                    <Typography variant="body2" color="text.secondary">(no containers)</Typography>
                )}
            </Box>
        </Box>
    );
