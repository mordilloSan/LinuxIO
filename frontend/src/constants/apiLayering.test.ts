...
// reads via `endpoint.useQuery` or `queryClient.fetchQuery(endpoint.queryOptions)`,
// writes via `endpoint.useJobAction`/`useJobStreamAction` or the
// background-jobs layer. The primitives — bare `linuxio.<handler>.<command>()`
// promise calls and raw React Query `useMutation` — live in the API layer.
