...
function formatFileSize(n?: number): string {
  ...
  let i = -1;
  let val = n;
  do { val /= 1024; i++; } while (Math.abs(val) >= 1024 && i < units.length - 1);
    ...
    <AppCardContent>
      ...
      >
        ...
        <div style={{ display: "flex" }}>
          ...
          <AppIconButton aria-label="Download Config" onClick={onDownloadConfig}>
      ...
      <div style={{ marginTop: 6 }}>
        <PeerCardRow label="Handshake" value={formatAgo(peer.last_handshake_unix)} />
        <PeerCardRow
          label="Rx"
          value={
            <>
              {formatFileSize(peer.rx_bytes)}{" "}
              <span style={{ color: "var(--app-palette-text-secondary)", fontWeight: 400 }}>
        ...
        <PeerCardRow label="Preshared Key" value={peer.preshared_key || "-"} wrap />
        <PeerCardRow label="Keep Alive" value={peer.persistent_keepalive ?? "-"} noDivider />
      </div>
    </AppCardContent>
  </FrostedCard>
);
