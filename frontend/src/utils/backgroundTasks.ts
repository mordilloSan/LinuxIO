...
/** Safe, route-declared public task metadata; raw requests are never available. */
export function taskMetadataObject(metadata: unknown): Record<string, unknown> {
  return metadata && typeof metadata === "object"
    ? (metadata as Record<string, unknown>)
    : {};
}

export function taskMetadataIdentity(metadata: unknown): string[] {
  const identity = taskMetadataObject(metadata).identity;
