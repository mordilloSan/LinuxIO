...
} from "@/hooks/backgroundTasks/useAnimatedIndexerStats";
...
describe("useAnimatedIndexerStats", () => {
  ...
  it("smoothly catches the displayed counters up to a confirmed SSE sample", () => {
    ...
    const { result, rerender } = renderHook(
      ({ stats, enabled, taskId }: HookProps) =>
        useAnimatedIndexerStats(stats, { enabled, taskId }),
      {
        initialProps: { enabled: true, taskId: "task-1", stats: initial },
      },
    );

    rerender({ enabled: true, taskId: "task-1", stats: target });
