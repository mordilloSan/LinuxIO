...
export function useDownloadTasks(runtime: BackgroundTaskRuntime) {
  const { set: setDownloads, read: readTasks } = runtime.tasks.downloads;
  ...
  // download can push hundreds a second); coalesce them into one cache update
  ...
  const startDownload = async (paths: string[]) => {
    ...
    try {
      ...
      downloadLabelBase = allocateDownloadLabelBase(candidateLabelBase, reqId);
  ...
  const cancelDownload = (id: string) => {
    const download = readTasks().find((d) => d.id === id);
    if (download) {
