...
import type { Upload } from "@/types/backgroundTasks";
...
export function useUploadTasks(
  ...
  // BackgroundTasksProvider (and the actions context identity) decoupled from
  // config changes.
  getChunkSize: () => number,
) {
  const { set: setUploads, read: readTasks } = runtime.tasks.uploads;
  const {
  ...
  /**
   * Task-backed batch upload: one bridge task and one data stream carry the
   * whole selection. The manifest (paths + sizes) travels in the task request,
   * so the stream is just every file's bytes back-to-back in manifest order —
   * the bridge splits them on the manifest boundaries.
   */
  const uploadBatchViaStream = async (
    ...
    const pendingUploadKey = taskIdentityKey(
      TaskTypes.TASK_TYPE_FILE_UPLOAD_BATCH,
      [
        request.destination,
    ...
    pendingLocalTaskKeysRef.current.add(pendingUploadKey);

    let task: TaskSnapshot;
    try {
      task = await linuxio.filebrowser.upload_batch(request);
    } catch (error) {
      pendingLocalTaskKeysRef.current.delete(pendingUploadKey);
      return {
        success: false,
        error:
    ...
    // Read the id once. `task` is a `let` assigned across an `await` and the
    ...
    updateUpload(uploadId, { taskId });
    activeFileTransferTaskIdsRef.current.add(taskId);
    pendingLocalTaskKeysRef.current.delete(pendingUploadKey);

    const stream = openTaskDataStream(taskId, 0);
    if (!stream) {
      activeFileTransferTaskIdsRef.current.delete(taskId);
      cancelBridgeTask(taskId);
    ...
    return new Promise<BatchStreamOutcome>((resolve) => {
      ...
      const resolveSafe = (outcome: BatchStreamOutcome) => {
        ...
        activeFileTransferTaskIdsRef.current.delete(taskId);
        // Free the bridge task slot when abandoning an incomplete upload. On a
        // stream error/early close the bridge parks the task in
        // `waiting_for_client` (to allow resume) rather than failing it, so
        // without an explicit cancel the task lingers and holds one of the
  ...
  const cancelUpload = (id: string) => {
    const upload = readTasks().find((u) => u.id === id);
    if (upload) {
      ...
      if (
        upload.taskId &&
        activeFileTransferTaskIdsRef.current.has(upload.taskId)
      ) {
        activeFileTransferTaskIdsRef.current.delete(upload.taskId);
        cancelBridgeTask(upload.taskId);
      }
